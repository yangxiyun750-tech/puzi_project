...
The optional `.zcode/config.json` compatibility profile is `SCORE_REBUILD`. It enables only `orchestral-score-rebuild`. Other profiles remain disabled unless a developer explicitly selects them.

## Codex

Open the repository root as the project. Confirm that the agent can see the project-local `orchestral-score-rebuild` Skill, then request it explicitly for score reconstruction. If the installed Codex surface does not discover `.agents/skills`, provide the absolute path to `SKILL.md` or install/copy the Skill using that Codex version's documented project-Skill mechanism. Do not depend on a mutable user-global Skill copy.
